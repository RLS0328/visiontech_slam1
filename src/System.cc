...
System::System(const string &strVocFile, const string &strSettingsFile,
	...
	const bool bReuse, const string &mapFilePath
	):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)),
	mbReset(false),mbActivateLocalizationMode(bReuse),
        mbDeactivateLocalizationMode(false),mMapPath(mapFilePath)
{
    ...
    //Create the Map
    mpMap = new Map();
    mMapSaveLoad = new MapSaveLoad(mpMap);
	if (bReuse)
	{
	...
	//mpKeyFrameDatabase->set_vocab(mpVocabulary);
        ...
        mSystemSetting->LoadSystemSetting(strSettingsFile.c_str());
        mMapSaveLoad->Load(mapFilePath.c_str(),mSystemSetting,mpKeyFrameDatabase);
/*
...
*/
    ...
    sched_param sch2;
    sch2.sched_priority = 20;
    pthread_setschedparam((*mptLocalMapping).native_handle(), SCHED_RR, &sch2);
    ...
    void System::SaveTrajectory(const string &filename)
    {
                    ...
                    lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
        {
            ...
            f << setprecision(6) <<( (*lT) * 10000) + 1111110000000000000 << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
        }
        f.close();
...
//int nf = 0;
//float ms1 = 0;
cv::Mat System::TrackMonocular(const cv::Mat &im, const std::vector<cv::KeyPoint> &msKeys, const cv::Mat &msDescriptors,const double &timestamp)
{
    if(mSensor!=MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular." << endl;
...
//    gettimeofday(&time0,NULL);
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,msKeys,msDescriptors,timestamp);
//    gettimeofday(&time1,NULL);
//   ms1 = (time1.tv_sec*1000000+time1.tv_usec) - (time0.tv_sec*1000000+time0.tv_usec);
//    nf = (nf+1)%100;
//			if(!nf)
//			{
//				cout<<"time = "<<(int)ms1<<endl;
//				ms1 = 0;
//			}
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
...
{
    SaveMap(mMapPath);
    //SaveMapPointAxis("MapPoint_Axis.txt");
}
