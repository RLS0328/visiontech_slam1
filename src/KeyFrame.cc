...
namespace ORB_SLAM2 {
    ...
    void KeyFrame::SetBadFlag() {
        static ofstream keyframe("./keyframe.log");
        {
            unique_lock<mutex> lock(mMutexConnections);
            if (mnId == 0)
                return;
        ...
        keyframe<<"SetBadFlag id"<<mnId<<endl;
        for (map<KeyFrame *, int>::iterator mit = mConnectedKeyFrameWeights.begin(), mend = mConnectedKeyFrameWeights.end();
             mit != mend; mit++) {
            //if(mit->first != NULL) {
                keyframe<<"connection id"<<mit->first->mnId<<"\t";
                ...
                mvpMapPoints[i]->EraseObservation(this);
        {
                    ...
                    for (size_t i = 0, iend = vpConnected.size(); i < iend; i++) {
                        for (set<KeyFrame *>::iterator spcit = sParentCandidates.begin(), spcend = sParentCandidates.end();
                            ...
                            if (vpConnected[i] == 0) {
                                ...
                                cout
                                        << "KeyFrame::SetBadFlag() : I'm just hacking it! Somewhere else must go wrong!"
                                        << endl;
                                continue;
                            }
                             */
            ...
            if (!mspChildrens.empty()) {
                for (set<KeyFrame *>::iterator sit = mspChildrens.begin(); sit != mspChildrens.end(); sit++) {
                    //if (mpParent) (*sit)->ChangeParent(mpParent);
        ...
        //if(mpKeyFrameDB != NULL) {
            mpKeyFrameDB->erase(this);
        //}
        keyframe<<"SetBadFlag End id"<<mnId<<endl;
