...
LoopClosing::LoopClosing(Map *pMap, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale):
    ...
    mbStopGBA(false), mbFixScale(bFixScale), mpMatchedKF(NULL),mpThreadGBA(NULL),mnFullBAIdx(0)
{
...
/*
    for(size_t i=0; i<vpKFs.size(); i++)
    {
      ...
      if((pKF->mnId) == 0)
      {
      cout<<"before lock"<<pKF->mTcwGBA<<endl;
      }
    }
*/
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            mpLocalMapper->RequestStop();            // Wait until Local Mapping has effectively stopped
...
void LoopClosing::addKeyFrame(KeyFrame *p) {
    mpKeyFrameDB->add(p);
}
